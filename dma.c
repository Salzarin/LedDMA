...
for (int j= 0; j<(led/40); j++){
	...
	uint32_t * srcArray[4096][256];//malloc(4*led/40);
	...
	for(int i = 0; i<((led/40)+1); i++){
		...
		srcArray[0][i] = (unsigned int*)virtSrcPage;
		memcpy(srcArray[0][i], data+i*4*3*40, 40*3*4);
	...
	unsigned int * srcData = srcArray[0][0];	
